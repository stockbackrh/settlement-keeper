...
export const pending = (limit = 20) => supa(`/rest/v1/stockback_claims?status=in.(accepted,queued)&order=created_at.asc&limit=${limit}&select=id,wallet,ticker,token_address,reward_usd,status`);
export const mark = (id, status, tx, amount, reason) => supa('/rest/v1/rpc/stockback_settle_claim', { method: 'POST', body: JSON.stringify({ p_secret: env.STOCKBACK_API_SECRET, p_id: id, p_status: status, p_tx: tx, p_amount: amount, p_reason: reason }) });
